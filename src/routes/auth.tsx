...
function AuthPage() {
  ...
  const handleGoogle = async () => {
    ...
    if (result.error) { toast.error("Falha no login com Google"); setLoading(false); }
  };
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-white">
          ...
          <div>
            <h2 className="font-display text-4xl font-bold leading-tight">
              A sala de aula que pensa, mede e evolui em tempo real.
            </h2>
            <p className="mt-4 max-w-md text-white/70">
              Junte-se a milhares de educadores transformando o aprendizado com IA pedagógica de ponta.
            </p>
          </div>
          <div className="text-xs text-white/50">© 2026 LyneKoto · Edu Intelligence</div>
        </div>
      </div>
      ...
      <div className="flex w-full lg:w-1/2 items-center justify-center bg-background px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {mode === "signin" ? "Entre para acessar seu studio." : "Comece grátis em segundos."}
          </p>
